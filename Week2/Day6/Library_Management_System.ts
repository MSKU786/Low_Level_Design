...
class ReservationQueue<T> {
  ...
  hasReservations(itemId: string) : boolean {
    ...
    class Book extends LibraryItem {
      ...
      constructor(id: string, title: string, year: number, ISBN: string, author: string) {
        super(id, title, year);
        this.ISBN = ISBN;
        this.author = author;
      }
    ...
    class DVD extends LibraryItem {
      ...
      constructor(id: string, title: string, year: number, duration: number, director: string) {
        super(id, title, year);
    ...
    class Member {
      ...
      constructor(id: string, name: string, email: string, phone: string, policy: BorrowingPolicy) {
        this.id = id;
        this.email = email;
        this.name = name;
        this.phone = phone;
    ...
    // Loan (encapsulates due date, fine status)
    ...
    class Loan {
      ...
      constructor(id: string, itemId: string, memberId: string, loanDurationDays: number) {
        this.id = id;
        this.itemId = itemId;
    ...
    class SearchService {
      ...
      searchByText(query: string): LibraryItem[] {
        ...
        return this.itemRepo.findAll().filter(item => item.getSearchableText().toLowerCase().includes(q));
      }
    ...
    class BorrowService {
      ...
      async borrowBook(memberId: string, itemId: string): Promise<Loan> {
        ...
        if (!member.canBorrow()) {
          throw new Error(`${member.name} has reached borrowing limit (${member.policy.maxItems})`);
        }
        ...
        const loan = new Loan(`loan_${Date.now()}`, itemId, memberId, member.policy.loanDurationDays);
        ...
        await this.notifier.sendNotification(member, `Borrowed: ${item.title} (loan id: ${loan.id})`);
    ...
    class ReturnService {
      ...
      async returnItem(loanId: string): Promise<{loan: Loan; fine: number}> {
        ...
        await this.notifier.sendNotification(member, `Returned: ${item.title}. Fine: ${fine}`);
        ...
        if (this.reservations.hasReservations(item.id)) {
          ...
          if (next) {
            ...
            if (nextMember) {
              await this.notifier.sendNotification(nextMember, `Item available: ${item.title}`);
            }
          }
        }
    ...
    async function demo() {
      ...
      // create sample data
      const book1 = new Book(id('book'), 'Clean Code', 2008, '9780132350884', 'Robert C. Martin');
      const dvd1 = new DVD(id('dvd'), 'Inception', 2010, 148, 'Christopher Nolan');

      itemRepo.save(book1);
      itemRepo.save(dvd1);

      const alice = new Member(id('mem'), 'Alice', 'alice@example.com', '1234567890', StudentPolicy);
      memberRepo.save(alice);

      const borrowService = new BorrowService(itemRepo, loanRepo, memberRepo, notifier);
      const returnService = new ReturnService(itemRepo, loanRepo, memberRepo, fineRegistry, reservations, notifier);

      console.log('Available items before borrow:', itemRepo.findAll().map(i => `${i.title} (${i.id})`));
